...
(function (eduVis) {
    ...
    var tool = {
        ...
        "name" : "NDBC_Data_Comparison_Tool",
        "description" : "NDBC Data Comparator.",
        ...
        "resources" : {
            ...
            "stylesheets_local" : [
                {
                    "name" : "toolstyle",
                    "src" : "tools/NDBC_Data_Comparison_Tool/NDBC_Data_Comparison_Tool.css"
                },
                {
        ...
        "data" : {},
        "target_div" : "NDBC_Data_Comparison_Tool",
        "tools" : {}

    };

    tool.NDBC_Data_Comparison_Tool = function( _target_div ){

        // reference data here
        var self = this;
        //self.target_div = _target_div;// || tool.target_div;
        ...
        this.observations = EduVis.ioos.getObservationObj(
            [
        ...
        //this.evtool.configurationParse( self.configuration.custom, customConfiguration );
        ...
        self.controls = {
            ...
            "station_list" : {
                ...
                "default_value" : self.configuration.station_list
            }
        };

        //create station list from control

        self.stations = EduVis.ioos.stationListLB( self.configuration.station_list );

        self.uiToolInterface();
    ...
    tool.init = function() {
        ...
        this.target_div = this.dom_target;

        // todo: include instance in call
        console.log("init at " + this.dom_target);
        this.NDBC_Data_Comparison_Tool(this.dom_target);



    },
    tool.uiToolInterface = function ( divId ) {
        //"use strict";
    ...
    tool.uiDimensions = function() {
        ...
        var self = this,
            ...
            container = settings.container.layout,
            chart = settings.chart.layout,
            controls = settings.controls.layout,
            id = settings.domID;
    ...
    tool.uiChart = function () {
        ...
        var self = this,
            ...
            id = settings.domID,
            chart_title = self.observations[config.variable1].name + " vs. " + self.observations[config.variable2].name;
    ...
    tool.dataParse = function ( ds_name, calcMean, calcStdDev ) {
        ...
        var self = this,
            ...
            ds = self.settings.datasets[ds_name];
    ...
    tool.dataDatasetInit = function ( station, variable, dateStart, dateEnd, url ) {
        var self = this;
        return {
            "station"     : station,
            "variable"    : variable,
    ...
    tool.dataDownload = function (downloadType) {
        ...
        d3.csv( url1, function (data) {
            ...
            var ds = self.settings.datasets["variable1"];

            if(data.length > 0){

                console.log("...data.. in response 1", data, data.length)
                ...
                // notify no data!?
                alert("No data is available for " + ds.colY)
                //todo: need better notification and fall back for invalid datasets!
            }
        ...
        d3.csv( url2, function (data) {
            ...
            var ds = self.settings.datasets["variable2"];

            if(data.length > 0){

                ds.data = data;

                console.log(".....dataset variable 2......:", ds);
    ...
    tool.dataInitialize = function ( ) {
        ...
        var self = this,
            dataset = self.settings.datasets["scatter"],

            chartView = self.configuration.chartView,
            ...
            tooltip = self.settings.chart.tooltip,
        ...
        if (chartView == "timeseries") {
            ...
            console.log(".... chartView -> Timeseries ...", "chart -> ",chart)
            ...
            var fmt = self.settings.formats.tooltip_num;
    ...
    //            .text(label2);
        ...
        console.log(".. CHART timeseries or scatter--> ", chart);
    },
    tool.transitionChart = function ( ){
        ...
        var fmt = self.settings.formats.linearModel;
    ...
    tool.dataParseDatasets = function ( ) {

        var self = this,
            ds = self.settings.datasets,
            ds1 = ds["variable1"],
            ds2 = ds["variable2"],
            var1 = ds1.variable,
            var2 = ds2.variable,
        ...
        var ctrl_button_scatter = $("<div></div>")
            ...
            $("<a></a>")
                ...
                .on("click",function(){
                    ...
                    self.configuration.chartView = "scatterplot";
                    self.transitionChart();
        ...
        var ctrl_button_timeseries = $("<div></div>")
            ...
            $("<a></a>")
                ...
                .on("click", function () {
                    ...
                    self.configuration.chartView = "timeseries";
                    self.transitionChart();
    ...
    tool.linear_regression = function(x, y){
    ...
    tool.update_variables = function(variable){
    ...
    tool.customization_update = function () {
        ...
        // todo: remove internal configuration updates
    ...
    tool.timeseries_mouseover = function (d,colX,colY,units,tooltip_label) {

        var self = this,
            date_format = d3.time.format("%m/%d/%Y-%H:%M"),
            fmt = self.settings.formats.tooltip_num;

        return self.settings.chart.tooltip.style("visibility", "visible")
            .attr("class","label label-"+ tooltip_label )
            .html(date_format(d[colX]) + " - <b>" + fmt(d[colY]) + units + "</b>");
    },

    tool.scatter_mouseover = function (d, col1, col2, units1, units2, observation1, observation2, tooltip_label) {
        console.log("scatter mouseover");

        var self = this,
            fmt_date = self.settings.formats.tooltip_date,
            fmt_num = self.settings.formats.tooltip_num;
    ...
    tool.mouseOut = function () {
        ...
        return self.settings.chart.tooltip.style("visibility", "hidden");
    };


    // extend base object with tool.. need to be able to leave options out of tool configuration file.
    // instances will be saved in EduVis.tool.instance[ tool, instance, case? (better word?) ]
